...
### Fixed
- **Job-scoped routines runner-route on spawnless instances — cloud Finalize no longer dies `[spawn error] spawn claude ENOENT` (owner /bug report 2026-07-22).** The pg/Railway image ships no claude binary, so every job-scoped routine click (Finalize, Draft, Interview prep, …) spawned a doomed local process — even though the hybrid runner lane already whitelists those kinds end to end. `startRun` now routes a job-scoped whitelisted runner kind into the laptop runner queue under the same instance-level dispatch decision as SIM-535's source discovery (`sourceRunDispatch` → `agentRunDispatch`; `JOBHUNT_SOURCE_DISPATCH` env seam unchanged). The run record stays in the ordinary runs Map, so the run panel poll, per-(routine, jobId) duplicate guard, batch drain, activity log and notifications are unchanged; runner progress lines fold through the same stream-json parser as a local spawn. Terminal state closes push-style the moment the runner's result lands (after its artifacts, so the drafted→ready auto-advance sees the new CV), with a lazy dead-laptop reconcile at the launch endpoints so a wedged run can never hold the duplicate guard or a concurrency slot forever. Stop on a runner-routed run is an honest 409 (the cloud holds no channel into the laptop, outbound-only by design) instead of a silent no-op. Ticket-scoped routines are not runner kinds and keep the local path. Tests: `tests/routine-runner-dispatch.test.js` (dispatch, guard, claim→progress→artifact→result, auto-advance, failure honesty, dead-laptop reconcile; spawn proven never reached).
...
## [0.38.3] - 2026-07-21 12:03 ET

Live security fix (SIM-466), auth-case-fix lane.

### Security
- **Auth-gate case-sensitivity bypass, closed (SIM-466).** Express matches routes case-insensitively by default, so a case-variant path (`GET /API/config`) skipped the auth gate's `req.path.startsWith("/api/")` check (case-sensitive, false on the uppercase variant) yet still matched the lowercase-registered handler — an anonymous-read bypass, LIVE (guardian finding 2026-07-21, twin of the SSC hub fix at `5b81e6a`). Fixed on both sides: `server/index.js` now sets `app.set("case sensitive routing", true)` so a case-variant path can never reach a lowercase-registered handler, and `createAuthGate` (`server/auth.js`) now matches the `/api/` prefix case-insensitively (default-deny) so a case-variant `/api/*` path is gated (401) rather than merely falling through to a 404. Regression test proves `GET /API/config` → 401 with auth on; red-checked against both halves of the fix.
