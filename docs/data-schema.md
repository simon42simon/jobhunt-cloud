...
### 7.0.1 The SIM-393 export-snapshot lane between the two authorized stores (I5; the MIRROR lane, I6, is RETIRED - see below)

Per the SIM-393 design gate (`company-os/audit/2026-07-17-sim393-vault-cloud-dataflow-design.md`, Owner amendment v2) and its guardian delta review (`company-os/audit/2026-07-18-sim393-v24-mirror-delta-review.md`), data moved between the owner's two authorized stores (the private cloud instance of §7.0 and this laptop) over dedicated, least-privilege token lanes. Each lane's disclosure landed in the same wave as its code (contract §6 / guardian GC-5): the ingest half shipped DORMANT with I1 (the insert-only `/api/sync/*` surface + `SYNC_TOKEN`, activated only at the one-shot transition import, I3); the export-snapshot half (landed with I5) is disclosed below.

**The cloud→vault MIRROR lane (I6) is RETIRED (2026-07-23, SIM-614).** By owner directive the OneDrive vault is no longer part of the jobhunt product loop at all - jobhunt-cloud (Postgres + the board) is now the only jobs/artifacts surface. The scheduled task that ran the mirror client was stopped and disabled in-console before this code removal, and the code itself was deleted outright, not merely disabled: `ops/mirror-vault.mjs`, the `/api/mirror/*` routes (the long-poll change feed, the raw job-detail read, the guarded file reader, the `POST /api/mirror/runs` report), `mirrorAuth`, and the `MIRROR_TOKEN`/`MIRROR_TOKEN_HASH` credential (including its demo-isolation boot gate) are all gone from this repo. The vault Jobs tree is consequently a FROZEN, un-updated copy of whatever it held at retirement - never written to again, by anything. `docs/changelog.md` carries the as-shipped history of I6 (its landing, its write semantics, its guardian conditions) as a historical record; this section no longer documents any of that as live behavior.

**The EXPORT-SNAPSHOT lane, landed with I5 (design section D; a point-in-time, verified, all-domain DR artifact - never a replacement for the mirror that used to exist):**

- **The lane:** a laptop-side client (`ops/export-snapshot.mjs`) makes one outbound, READ-ONLY pull of the ENTIRE cloud dataset into the file layout the local app already understands (`Jobs/<folder>/<Role>.md` reconstructed byte-faithfully from raw frontmatter + body, companion files, `tasks.yaml`, `requests.yaml`, `attachments/`, `activity-log.jsonl`, `usage-telemetry.jsonl`, `notify-state.json`, `job-chats.json`, `discovery-sources.yaml`), plus a `snapshot-manifest.json` (per-file sha256, domain counts, cloud app version, timestamp). Outbound-only (GC-6): no listener, pinned host, https-only, redirect-refusing, TLS-bypass-refusing. The pinned-host `createApi`, the overlap-guard lockfile, and the `<Role>.md` reconstruction now live in the shared `ops/cloud-client.mjs` module (extracted from `ops/mirror-vault.mjs` before that file was deleted, so removing the mirror lane never touched this still-live code).
- **The credential:** `EXPORT_TOKEN` — a **read-only token over the full dataset** (the guardian's C2/RR-7 register entry: theft = full read of the career dataset, integrity intact). Mitigation set as reviewed: the export surface is **GET-only, enforced as middleware** (any other verb 405s; the ONE sanctioned non-GET is the bounded `POST /api/export/runs` report line), constant-time verify against a sha256-only `EXPORT_TOKEN_HASH` cloud-side (plaintext lives in `~/.ssc-secrets` on the laptop only), per-IP failure rate-limit, failed attempts feeding the SIM-386 visibility pipeline as `surface:"export"`, and independent rotation — separate from `SYNC_TOKEN`, `RUNNER_TOKEN`, and the passphrase; no credential grants another's powers (cross-auth matrix proven both directions: `EXPORT_TOKEN` 401s every sync route, and `SYNC_TOKEN`/`RUNNER_TOKEN` 401 every export route). The demo instance refuses to boot if any export material is present (GC-3).
- **The routes:** `/api/export/*` — gap-fill reads added ONLY because no token-authed API served these domains (the session GETs are cookie-gated): `meta`, `manifest` (the same jobs+files hash view the sync lane uses), `jobs/:id` (raw detail, via `store.mirrorJobDetail` - a name kept from the retired mirror lane; export is now its only caller), `jobs/:id/files/:name` (the guarded-reader idiom), `tasks`, `requests`, `chats`, `notify-state`, `sources`, `activity`, `telemetry`, `attachments/:taskId/:file` — each a thin read-only wrapper over an existing store read.
- **Detection on use (GC-2):** every export run appends one structured activity line (`kind:"export"`, `event:"export-run"`, with counts + a `verified` flag), so an export the owner did not schedule is visible in-app.
- **GC-1 (HIGH) — client-side containment:** every cloud-supplied name (job folder, file name, attachment task id + file name, any id used in a path) passes the shared `server/name-safety.js` validation AND `resolveInside`-the-snapshot-root containment BEFORE any write; a hostile name is refused with zero filesystem effect, and a snapshot containing any refusal is never certified. Server-side validation does not substitute — the duty lives in the client, against the compromised-cloud case.
- **Where snapshots land:** `%SSC_ROOT%\data\jobhunt\cloud-snapshots\<UTC-timestamp>\`, append-only, inside the existing data zone (the zone's `index.md`/`log.md` record the subfolder and its writer). Deliberately NOT in OneDrive: snapshots contain domains (chats, activity, telemetry, tasks) homed outside any synced store; putting them in the vault would silently add Microsoft as a storage party for data classes that never lived there.
- **`VERIFIED` semantics:** the marker is written ONLY after a verification pass that re-fetches the manifest + every domain and asserts an exact match against the bytes re-read from disk, with zero refused names and zero integrity conflicts. A snapshot without `VERIFIED` is treated as garbage by the restore drill (SIM-64: boot a FileStore over the snapshot, or re-run `ops/migrate-data.mjs` from it).
- **Never deletes / prune stays cold (GC-7):** the snapshot pass contains no delete path; `--prune` is a separate, owner-invoked mode that REFUSES to run unless the owner has set `retention.keep` in config, always protects the newest VERIFIED snapshot(s), never touches an unverified snapshot, and is wired into no schedule. Ship default: keep everything.
...
| Version | Date | Change |
| --- | --- | --- |
| 8 (pending release cut) | 2026-07-18 | **SIM-393 I5 export-snapshot disclosure (§7.0.1 export half) + I4 drawer upload.** The export lane is disclosed in full: `EXPORT_TOKEN` (read-only over the full dataset, C2/RR-7 register class; GET-only enforced as middleware with the one bounded `POST /api/export/runs` report line; sha256 verify-hash only cloud-side; demo boot-refusal per GC-3), the `/api/export/*` gap-fill routes, the laptop client `ops/export-snapshot.mjs` (GC-6 pinned-host outbound-only posture reused from the mirror client; GC-1 client-side name-safety + `resolveInside` containment on every cloud-supplied name), the snapshot location `%SSC_ROOT%\data\jobhunt\cloud-snapshots\<UTC-ts>\` (append-only, outside OneDrive by design), `VERIFIED` marker semantics (written only after the re-fetch verification pass), the per-run activity line (GC-2), and the cold-by-default `--prune` (GC-7: refuses without owner-set `retention.keep`, newest VERIFIED always protected, never scheduled). Cross-auth matrix extended with the EXPORT↔MIRROR directions per the I6 landing-check carried condition. I4: the drawer upload (`POST /api/jobs/:id/files`, session wall, insert-only `addJobFileUnique` with unique-name derivation; GC-4 demo caps ≤ 1 MB + 6 files/job) rides §7.1's write-path row set. Release-manager confirms the app version at cut |
| 7 (pending release cut) | 2026-07-18 | **SIM-393 I6 mirror-lane disclosure (new §7.0.1, mirror half).** The vault Jobs tree is re-designated a passive one-way replica of the cloud (Owner amendment v2, 2026-07-17); the standing cloud→vault mirror lane is disclosed in full: `MIRROR_TOKEN` (jobs-domain GET-only, guardian GC-9), the standing outbound long-poll connection (`GET /api/mirror/changes`, triggers only), overwrite-own-writes-only semantics (three-way sha check against `%LOCALAPPDATA%\ssc\jobhunt-mirror-state.json`), the absolute no-delete rule, and the accepted accumulation of stale mirror copies. Same-wave with the I6 code per contract §6 / GC-5, gated by the guardian delta review `company-os/audit/2026-07-18-sim393-v24-mirror-delta-review.md`; activation additionally requires the GC-11 decision-log record. Ingest half (I1/I3) noted as shipped-dormant; export half lands with I5 |
