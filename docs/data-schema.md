...
**The EXPORT-SNAPSHOT half, landed with I5 (design section D; the mirror is NOT a backup — it covers the Jobs tree only and tracks the cloud; the snapshot is the point-in-time, verified, all-domain DR artifact):**

- **The lane:** a laptop-side client (`ops/export-snapshot.mjs`) makes one outbound, READ-ONLY pull of the ENTIRE cloud dataset into the file layout the local app already understands (`Jobs/<folder>/<Role>.md` reconstructed byte-faithfully from raw frontmatter + body, companion files, `tasks.yaml`, `requests.yaml`, `attachments/`, `activity-log.jsonl`, `usage-telemetry.jsonl`, `notify-state.json`, `job-chats.json`, `discovery-sources.yaml`), plus a `snapshot-manifest.json` (per-file sha256, domain counts, cloud app version, timestamp). Outbound-only, same GC-6 posture as the mirror client (whose `createApi` it literally reuses): no listener, pinned host, https-only, redirect-refusing, TLS-bypass-refusing.
- **The credential:** `EXPORT_TOKEN` — a **read-only token over the full dataset** (the guardian's C2/RR-7 register entry: theft = full read of the career dataset, integrity intact). Mitigation set as reviewed: the export surface is **GET-only, enforced as middleware** (any other verb 405s; the ONE sanctioned non-GET is the bounded `POST /api/export/runs` report line), constant-time verify against a sha256-only `EXPORT_TOKEN_HASH` cloud-side (plaintext lives in `~/.ssc-secrets` on the laptop only), per-IP failure rate-limit, failed attempts feeding the SIM-386 visibility pipeline as `surface:"export"`, and independent rotation — separate from `SYNC_TOKEN`, `MIRROR_TOKEN`, `RUNNER_TOKEN`, and the passphrase; no credential grants another's powers (cross-auth matrix proven both directions incl. the mirror lane: `EXPORT_TOKEN` 401s every sync and mirror route, and those tokens 401 every export route). The demo instance refuses to boot if any export material is present (GC-3).
- **The routes:** `/api/export/*` — gap-fill reads added ONLY because no token-authed API served these domains (the session GETs are cookie-gated; the mirror lane is jobs-domain-scoped by GC-9 and was not widened): `meta`, `manifest` (the same jobs+files hash view the sync/mirror lanes use), `jobs/:id` (raw detail), `jobs/:id/files/:name` (the guarded-reader idiom), `tasks`, `requests`, `chats`, `notify-state`, `sources`, `activity`, `telemetry`, `attachments/:taskId/:file` — each a thin read-only wrapper over an existing store read.
