...
6. **Generate the CV.** Build from the facts for the chosen track - do not invent content, do not copy a prior generated CV. **Track-pack first (SIM-597):** run the cache protocol in "Facts + track-pack access" - on a HIT, take the facts-stable blocks from `pack.blocks` and only generate the posting-tailored layer; on a MISS, build the blocks below from the facts API, then PUT the pack:
   ...
   - Experience: each job newest-first, using `achievements[track]`. Reorder/prioritize bullets to surface the posting's keywords first; you may drop the weakest bullet for length, but never add a bullet that is not in the facts. Aim to fill **two full pages — and never more** (the draft-time gate rejects a CV over 2 pages, step 8b): lengthen the experience section with more relevant bullets and roles rather than leaving a half-empty second page.
   ...
   - **Fill two pages (80-100% of page 2), never a third.** The CV should fill close to two full pages; lengthen experience/involvement bullets rather than leaving page 2 half-empty. The renderer warns if it is not exactly 2 pages — heed that warning in BOTH directions (step 8b makes over-cap terminal).
...
7. **Generate the cover letter.** From the `cover_letter` facts for the chosen track (on a track-pack HIT the openings/hero-phrase blocks come from `pack.blocks`; only the posting-specific middle is fresh):
   ...
   - Address to the named hiring manager if the posting gives one, else "Dear Hiring Committee". Tighten to **one page maximum** — the draft-time gate rejects a cover letter over 1 page (step 8b), so cut the weakest paragraph content rather than run long.
...
8. **Render the documents (.docx only).** Set `outdir`, `cv_basename` (`Simon Kim - CV - <Job Title>`), and `cl_basename` in the JSON, then run with `--no-pdf`:
   ...
   It writes the CV and cover letter as `.docx` (python-docx) in Simon's real format (Times New Roman, ruled ALL-CAPS headings; see [[cv-cover-letter-workflow]] for the spec). **No PDF at first draft** - finalize-job renders the submission PDFs after Simon answers the gaps page (this also keeps the batch fast and frees MS Word). To revise, edit the JSON and re-run; it overwrites in place.

8b. **Verify page caps BEFORE finishing (SIM-598 gate is draft-time — over-cap artifacts are REJECTED).** Since 2026-07-23 (v0.41.0) the cloud's fail-closed quality gate runs on every artifact post at DRAFT time, estimating pages from the `.docx` word count: **CV ≤ 2 pages, cover letter ≤ 1 page**; an over-cap `.docx` is rejected (400) and, since v0.42.0 (SIM-615/613), that rejection FAILS the whole run loudly — there is no finalize-time second chance for a first draft. So before you consider step 8 done: check the renderer's page count/warnings for BOTH documents; if the CV exceeds 2 pages or the cover exceeds 1, trim at the `application-content.json` level and re-render, repeating until both fit. **Trim like `tighten-docs` (owner rule, 2026-07-23):** first remove wasted words inside bullets/paragraphs (filler adverbs, redundant qualifiers, restated context) so no achievement is lost; only then drop the weakest whole bullet; never shrink fonts or margins. Never leave an over-cap `.docx` as the run's output. (Live incident 2026-07-23, run `aj-1784830740331-63550e94`: both artifacts gate-rejected because this skill still assumed the guard ran at finalize.)
