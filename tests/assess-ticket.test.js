...
beforeAll(async () => {
  ...
  // CLAUDE_BIN_PRESENT is false (honest degradation, no doomed spawn). spawn
  // itself is mocked below regardless, but CLAUDE_BIN_PRESENT is a real
  // fs.existsSync check made at module load - a CI runner has no claude
  // binary anywhere, so without this seam every launch in this file would
  // short-circuit before ever reaching the mock. JOBHUNT_CLAUDE_BIN points it
  // at a controlled, always-present path (mirrors JOBHUNT_PYTHON).
